...
func UploadImage(filePath string, libraryRef string, libraryURL string, authToken string, description string) error {
	...
	glog.V(2).Infof("Image hash computed as %s\n", imageHash)
	...
	if !found {
		glog.V(1).Infof("Entity %s does not exist in library - creating it.\n", entityName)
	...
	if !found {
		glog.V(1).Infof("Collection %s does not exist in library - creating it.\n", collectionName)
	...
	if !found {
		glog.V(1).Infof("Container %s does not exist in library - creating it.\n", containerName)
	...
	if !found {
		glog.V(1).Infof("Image %s does not exist in library - creating it.\n", imageHash)
	...
	if !image.Uploaded {
		glog.Infof("Now uploading %s to the library\n", filePath)
...
func postFile(baseURL string, authToken string, filePath string, imageID string) error {
	...
	glog.V(2).Infof("postFile calling %s\n", postURL)
